...
"""Tests for the render command (repo_scanner.commands.render_cmd)."""
...
def test_round_trips_json_through_sqlite() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert run_render(src, fmt="sqlite", output_path=db) == 0

        table = io.StringIO()
        with redirect_stdout(table):
            assert run_render(db, fmt="table") == 0  # sqlite input -> table
        assert "a.py:3" in table.getvalue()

        rendered = io.StringIO()
        with redirect_stdout(rendered):
            assert run_render(db, fmt="json") == 0  # sqlite input -> json
...
def test_sqlite_output_requires_an_output_file() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert run_render(src, fmt="sqlite") == 1  # emit fails: no -o FILE


def test_missing_or_unrecognized_input_is_a_usage_error() -> None:
    assert run_render("/no/such/file.json") == 2
    with tempfile.TemporaryDirectory() as directory:
        path = _write(directory, "junk.txt", "not a report")
        assert run_render(path) == 2
